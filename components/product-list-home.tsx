...
const moveRight = () => {
  ...
  return (
    <div className="space-y-4 2xl:pt-8 ">
      ...
      <h2 className="font-bold text-5xl font-['Brevia'] text-cyan-700">Productos Destacados</h2>
      <Link
                  href="/products"
      ...
      {items.length === 0 && <NoResults />}
        ...
        <button onClick={moveLeft} className="carousel-button h-16 w-16 text-3xl text-white items-center justify-center text-center rounded-full bg-cyan-700 left" aria-label="Anterior">
        {'<'}
</button>
       <button onClick={moveRight} className="carousel-button  h-16 w-16 text-3xl text-white items-center justify-center text-center rounded-full bg-cyan-700 right" aria-label="Siguiente">
          {'>'}
</button>
        </div>
      <div className="scene">
